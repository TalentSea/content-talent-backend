...
class GrowthMetric(BaseModel):
    ...
    previous: float | int = Field(
        ..., description="Value in immediately preceding equivalent period"
    )
    growth_percentage: float = Field(
        ..., description="Period-over-period percentage delta (-100.0% to +100.0%+)"
    )
...
class DashboardStatsResponse(BaseModel):
    ...
    total_users: GrowthMetric = Field(
        ..., description="Registered audience accounts telemetry and growth"
    )
    total_subscribers: GrowthMetric = Field(
        ..., description="Active paying subscribers telemetry and growth"
    )
    total_content: ContentInventoryBreakdown = Field(
        ..., description="Creator content catalog inventory breakdown"
    )
...
class AnalyticsDataPoint(BaseModel):
    ...
    date: str = Field(
        ..., description="ISO boundary date (YYYY-MM-DD) for tooltips & machine sorting"
    )
    label: str = Field(
        ..., description="Human-readable chart label (e.g. '09 Sep', 'Week 36', 'Jun')"
    )
    users: int = Field(
        ..., description="New registered user signups within this interval"
    )
    subscribers: int = Field(
        ..., description="Active paying subscribers count as of this interval"
    )
    revenue: float = Field(
        ..., description="Total captured revenue in ₹ INR within this interval"
    )
...
class SubscriptionTierItem(BaseModel):
    ...
    subscribers: int = Field(
        ..., description="Active enrolled subscribers in this tier"
    )
    subscribers_percentage: float = Field(
        ..., description="Percentage of total subscribers rounded to 1 decimal place"
    )
    revenue: float = Field(
        ..., description="Actual captured revenue in ₹ INR during the selected period"
    )
    revenue_percentage: float = Field(
        ..., description="Percentage of total period revenue rounded to 1 decimal place"
    )
...
class SubscriptionBreakdownResponse(BaseModel):
    ...
    total_revenue: float = Field(
        ..., description="Total captured revenue in ₹ INR across all tiers in period"
    )
    tiers: list[SubscriptionTierItem] = Field(
        ..., description="Breakdown per subscription plan tier"
    )
...
class RecentActivityUserItem(BaseModel):
    ...
    email: str | None = Field(
        None, description="Email address or null for phone-based/guest registrations"
    )
    avatar_url: str | None = Field(None, description="CDN profile photo URL or null")
    plan_name: str | None = Field(
        None, description="Name of active subscription plan or null if non-paying"
    )
    is_paid: bool = Field(
        ..., description="True if user currently holds an active paid membership"
    )
    subscribed_at: datetime | None = Field(
        None, description="ISO timestamp when user subscribed, or null"
    )
    joined_at: datetime = Field(
        ..., description="ISO timestamp when account was registered"
    )
