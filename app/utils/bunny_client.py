...
def create_bunny_video(title: str) -> dict:
    ...
    url = (
        f"https://video.bunnycdn.com/library/{settings.BUNNY_STREAM_LIBRARY_ID}/videos"
    )
    headers = {
    ...
    try:
        response = requests.post(
            url, json={"title": title}, headers=headers, timeout=10
        )
...
def get_bunny_video_status(bunny_video_id: str) -> dict:
    ...
    headers = {"AccessKey": settings.BUNNY_STREAM_API_KEY, "Accept": "application/json"}
...
def delete_bunny_storage_file(file_path: str) -> bool:
    ...
    url = (
        f"https://storage.bunnycdn.com/{settings.BUNNY_STORAGE_ZONE_NAME}/{clean_path}"
    )
...
def upload_bunny_storage_file(
    file_path: str, file_bytes: bytes, content_type: str = "image/jpeg"
) -> bool:
    """
