...
def verify_google_id_token(id_token_str: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"Google OIDC verification error: {str(err)}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Invalid or expired Google OIDC identity token: {str(err)}"
        )
    except Exception as err:
        logger.error(f"Unexpected Google auth verification failure: {str(err)}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def verify_facebook_access_token(access_token_str: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if res.status_code != 200:
            raise ValueError(f"Facebook Graph API responded with status {res.status_code}: {res.text}")
        ...
        logger.warning(f"Facebook Graph API token validation error: {str(err)}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Invalid or expired Facebook access token: {str(err)}"
        )
    except Exception as err:
        logger.error(f"Unexpected Facebook verification failure: {str(err)}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
