...
@router.post(
    "/initiate",
    response_model=VideoInitiateResponse,
    status_code=status.HTTP_201_CREATED,
)
def initiate_video(payload: VideoInitiateRequest, current_user: CurrentAdmin):
    """
...
@router.get(
    "",
    response_model=PaginatedResponse[VideoListItemResponse],
    status_code=status.HTTP_200_OK,
)
def list_videos(
    current_user: CurrentAdmin,
    status_filter: str | None = Query(
        None, alias="status", description="Filter by state: published, draft, scheduled"
    ),
    category: str | None = Query(None, description="Filter by category slug"),
    search: str | None = Query(None, description="Search video titles by substring"),
    sort: str | None = Query(
        "newest", description="Sort order: newest, oldest, views, title"
    ),
    date_from: str | None = Query(
        None, alias="dateFrom", description="Filter creation start date (ISO string)"
    ),
...
def get_video_details(video_id: int, current_user: CurrentAdmin):
    """
...
@router.patch(
    "/{video_id}", response_model=VideoUpdateResponse, status_code=status.HTTP_200_OK
)
def update_video_metadata(
    video_id: int, payload: VideoUpdateRequest, current_user: CurrentAdmin
):
    """
...
@router.patch(
    "/{video_id}/thumbnails/select-main",
    response_model=ActionSuccessResponse,
    status_code=status.HTTP_200_OK,
)
def select_main_thumbnail(
    video_id: int, payload: SelectMainThumbnailRequest, current_user: CurrentAdmin
):
    """
...
def delete_thumbnail(
    video_id: int, payload: DeleteThumbnailRequest, current_user: CurrentAdmin
):
    """
...
@router.delete(
    "/{video_id}", response_model=ActionSuccessResponse, status_code=status.HTTP_200_OK
)
def delete_video(video_id: int, current_user: CurrentAdmin):
    """
...
@router.post(
    "/bulk-delete", response_model=ActionSuccessResponse, status_code=status.HTTP_200_OK
)
def bulk_delete_videos(payload: BulkDeleteVideosRequest, current_user: CurrentAdmin):
    """
...
def publish_video(video_id: int, current_user: CurrentAdmin):
    """
...
def schedule_video(
    video_id: int, payload: VideoScheduleRequest, current_user: CurrentAdmin
):
    """
