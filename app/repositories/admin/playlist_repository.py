...
class PlaylistRepository:
    ...
    def create_playlist(
        self, playlist_data: dict, tenant_id: int, created_by: int | None = None
    ) -> Playlist:
        """
    ...
    def get_playlist_by_id(self, playlist_id: int, tenant_id: int) -> Playlist | None:
        ...
        return Playlist.get_or_none(
            (Playlist.id == playlist_id) & (Playlist.tenant == tenant_id)
        )
    ...
    def get_playlist_videos(
        ...
        """
        playlist = self.get_playlist_by_id(playlist_id, tenant_id)
        if not playlist:
            return None, [], 0
    ...
    def get_all_playlists_by_user(
        ...
        """
        Fetches a paginated, filtered, and sorted list of tenant playlists along with total count, video count, and saves count.
        """
        query = Playlist.select().where(Playlist.tenant == tenant_id)
    ...
    def update_playlist(
        self, playlist_id: int, tenant_id: int, update_data: dict
    ) -> Playlist | None:
        """
    ...
    def add_videos_to_playlist(
        self, playlist_id: int, tenant_id: int, video_ids: list[int]
    ) -> Playlist | None:
        """
        Adds an array of video IDs to the specified playlist if owned by tenant.
        """
        playlist = self.get_playlist_by_id(playlist_id, tenant_id)
        if not playlist:
            return None
    ...
    def remove_video_from_playlist(
        self, playlist_id: int, tenant_id: int, video_id: int
    ) -> bool:
        """
    ...
    def reorder_playlist_videos(
        self, playlist_id: int, tenant_id: int, video_orders: list[dict]
    ) -> bool:
        """
    ...
    def get_available_videos_for_playlist(
        ...
        """
        Executes query returning paginated, filtered, and sorted list of tenant videos NOT attached to the specified playlist.
        """
